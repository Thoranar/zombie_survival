...
export interface IEngineAdapter {
  ...
  drawHorizontalBar(
    ...
    opts?: { background?: string; fill?: string; border?: string }
  ...
  drawTrap(
    ...
    y: number,
    sizePx: number,
    opts: { icon: string; completed: boolean; hpRatio: number; selected: boolean }
  ): void;
...
export class Canvas2DEngine implements IEngineAdapter {
  ...
  public drawTrap(
    ...
    const hp = Math.max(0, Math.min(1, opts.hpRatio));

    const baseColor = opts.completed

      ? (hp > 0.66 ? '#795548' : hp > 0.33 ? '#6d4c41' : '#4e342e')

      : '#757575';

    const outlineColor = opts.selected ? '#ffd54f' : '#3e2723';
    ...
    this.ctx.moveTo(-spike, 0);

    this.ctx.lineTo(spike, 0);

    this.ctx.moveTo(0, -spike);

    this.ctx.lineTo(0, spike);
    ...
    if (opts.icon === 'spike') {
      ...
      this.ctx.moveTo(0, -inner);

      this.ctx.lineTo(inner * 0.6, 0);
  ...
  public drawHorizontalBar(
    ...
    const bg = opts.background ?? 'rgba(0,0,0,0.65)';

    const fill = opts.fill ?? '#ef5350';

    const border = opts.border ?? '#ffffff';

    const left = s.x - width / 2;

    const top = s.y - aboveCenterPx - height;
    ...
    this.ctx.fillRect(left, top, width * p, height);
